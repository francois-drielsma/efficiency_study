...
class CompareCutsConfig(CompareConfig):
    def __init__(self, beam, target_dir, top_labels, right_labels):
        ...
        self.conglomerate_list = [
            self.get_conglomerate_2("global_through_virtual_diffuser_us_r_8_0", None, "Radius at diffuser (upstream) [mm]", None, True, [0.5, 0.5, 0.9, 0.9], vertical([100], mod)),
            self.get_conglomerate_2("global_through_virtual_diffuser_ds_r_8_0", None, "Radius at diffuser (downstream) [mm]", None, True, [0.5, 0.5, 0.9, 0.9], vertical([100], mod)),
            self.get_conglomerate_2("tkd_n_tracks_11_0", None, "Number of tracks in TKD", None, True, [0.5, 0.5, 0.9, 0.9], vertical([0.5, 1.5], mod)),
            self.get_conglomerate_2("tkd_chi2_11_0", None, "#chi^{2}/D.o.F. in TKD", None, True, [0.5, 0.5, 0.9, 0.9], vertical([4], mod)),
            self.get_conglomerate_2("tkd_max_r_11_0", None, "Maximum radius in TKD stations [mm]", [0., 300.], True, [0.5, 0.5, 0.9, 0.9], vertical([150], mod)),
            self.get_conglomerate_2("tkd_p_12_0", None, "Momentum at TKD Reference Plane [MeV/c]", [50., 250.], True, [0.5, 0.5, 0.9, 0.9], vertical([90, 170], mod)),
            self.get_conglomerate_2("tku_chi2_8_0", None, "#chi^{2}/D.o.F. in TKU", None, True, [0.5, 0.5, 0.9, 0.9], vertical([4], mod)),
            self.get_conglomerate_2("tku_max_r_8_0", None, "Maximum radius in TKU stations [mm]", None, True, [0.5, 0.5, 0.9, 0.9], vertical([150], mod)),
            ...
            self.get_conglomerate_2("tku_p_8_0", None, "Momentum at TKU Reference Plane [MeV/c]", [50., 250.], [135, 145], [0.5, 0.5, 0.9, 0.9], vertical([135, 145], mod)),
            self.get_conglomerate_2("tof_tof0_n_sp_8_0", None, "Number of space points in ToF0", None, True, [0.5, 0.5, 0.9, 0.9], vertical([0.5, 1.5], mod)),
            self.get_conglomerate_2("tof_tof1_n_sp_8_0", None, "Number of space points in ToF1", None, True, [0.5, 0.5, 0.9, 0.9], vertical([0.5, 1.5], mod)),
            self.get_conglomerate_2("tof_tof01_8_0", None, "Time between ToF0 and ToF1 [ns]", [2.5, 5.], True, [0.5, 0.5, 0.9, 0.9], vertical([3.0, 3.5, 4.0, 4.5], mod)),
            self.get_conglomerate_2("tof_delta_tof01_9_0", None, "t(ToF01) - extrapolated t(ToF01) [ns]", None, True, [0.1, 0.5, 0.5, 0.9], mod),

            self.get_conglomerate_2("tku_scifi_n_planes_with_clusters_9_0", None, "Number of planes with clusters in TKU", None, True, [0.1, 0.5, 0.5, 0.9], modifiers = mod),
            ...
            self.get_conglomerate_2("tkd_scifi_n_planes_with_clusters_9_1", None, "Number of planes with clusters in TKD", None, True, [0.1, 0.5, 0.5, 0.9], modifiers = mod),
            ...
            self.get_conglomerate_2("tkd_scifi_n_planes_with_clusters_12_0", None, "Number of planes with clusters in TKD", None, True, [0.1, 0.5, 0.5, 0.9], modifiers = mod),
        ]
        self.data_caption = [[],]
        self.data_caption[0] = [
...
class CompareData2DConfig(CompareConfig):
    def __init__(self, beam, target_dir, top_labels, right_labels):
        self.setup(beam, target_dir, "data_plots/", "compare_data_2d/", True, False)
        mod = {
            "extra_labels":{
                "right":right_labels,
                "top":top_labels
...
class CompareData2DMCConfig(CompareConfig):
    def __init__(self, beam, target_dir, top_labels, right_labels):
        self.setup(beam, target_dir, "data_plots/", "compare_data_2d_mc/", False, True)
        mod = {
            "extra_labels":{
                "right":right_labels,
                "top":top_labels
...
class CompareOpticsMCConfig(CompareConfig):
    def __init__(self, beam, target_dir, top_labels, right_labels):
        self.setup(beam, target_dir, "optics_plots/", "compare_optics_mc/", False, True)
        graphs = ["_source_tku", "_source_tkd", "_virtual_absorber_centre",]
        for tof in ["_tof0", "_tof1", "_tof2"]:
            for element in "_us", "", "_ds":
                graphs.append(tof+element)
    ...
    def __init__(self, beam, target_dir, top_labels, right_labels):
        ...
        self.conglomerate_list = [
            ...
            self.get_conglomerate_graph("cdf_ratio*", "Reconstructed amplitude [mm]",
                                        ...
                                        ["CDF_Ratio stats", "CDF_Ratio sys"], x_range = [0.01, 99.9], y_range = [0.801, 1.599], replace_hist = True,
                                        graph_draw_option = ["p", "2"], graph_marker_style=[20, 20], graph_marker_color=[ROOT.kRed, ROOT.kRed], graph_draw_order=[1,0], modifiers=ratio_modifiers),
        ]
    ...
    def __init__(self, beam, target_dir, top_labels, right_labels):
        ...
        self.conglomerate_list = [
            ...
            self.get_conglomerate_graph("cdf_ratio*", "Reconstructed amplitude [mm]",
                                        ...
                                        ["CDF_Ratio stats", "CDF_Ratio sys"], x_range = [0.01, 99.9], y_range = [0.601, 1.599], replace_hist = True,
                                        graph_draw_option = ["p", "2"], graph_marker_style=[20, 20], graph_marker_color=[1, 1], graph_draw_order=[1,0], modifiers=ratio_modifiers),
        ]
...
def systematics():
    target_dir = "output/2017-02-Systematics-5/"
    dir_list = [
...
def main_paper(batch_level = 0):
    ...
    ROOT.gROOT.SetBatch(True)
    target_dir = "output/2017-02-7-v2/"
    batch_level = 0
...
def main_note(batch_level = 0):
    ...
    root_style.setup_gstyle()
    ROOT.gROOT.SetBatch(True)
    target_dir = "output/2017-02-drielsma-den=2.0_si=0.2/"
    batch_level = 0
    hide_root_errors = True
    do_cuts_summary = True
    if batch_level < 10 and hide_root_errors:
    ...
    systematics()
    my_dir_list = [
        ["2017-2.7_3-140_lH2_empty",],
    ]
    top_labels = ["4-140"]#, "6-140", "10-140"]
    right_labels = ["No\nabsorber"]#, "Empty\nLH2", "Full\nLH2", "LiH"]
    config_list = [CompareData1DConfig, CompareCutsConfig]#, CompareData2DMCConfig, CompareData2DConfig] #CompareData2DMCConfig, CompareData2DConfig] #, CompareOpticsMCConfig]#, CompareData1DConfig] #CompareOpticsConfig, CompareGlobalsConfig, CompareData2DConfig, CompareMCConfig, CompareCutsConfig, , ]
