...
def get_analysis(run_list, name, tof01_min_max, maus_version, data_dir, extrap, amplitude_source, p_bins, tkd_cut):
    ...
    return {
            ...
            "amplitude_source":"output/2017-02_1_mc/plots_"+amplitude_source+"/amplitude.json",
...
class Config(object):
    ...
    analyses = []#[8699, 8644, 8651, 8652, 8665, 8667, 8676, 8679, 8682]
    #analyses.append(get_analysis([8644], "6-140", 31., "MAUS-v2.9.1", data_dir, "tku_tp", "6-140_MC"))
    #analyses.append(get_analysis([8685, 8645, 8653, 8677, 8680, 8683, 8687, 8689, 8691, 8692, 8693], "10-140", 30., "MAUS-v2.9.1", data_dir, "tku_tp", "10-140_MC"))
    #[8681, 8643, 8650, 8664, 8675, 8678, 8684, 8686, 8688, 8690, 8694, 8695, 8696, 8697, 8698, 8709]
    for run in []: #[9726]:
        analyses.append(get_analysis([run], "2017-02-1 3-140-"+str(run), [27, 32], "MAUS-v3.0.0", data_dir, "3-140_MC", [[50, 200]], [30, 80]))
    for run in []: #[9727]:
        analyses.append(get_analysis([run], "2017-02-1 6-140-"+str(run), [27, 31], "MAUS-v3.0.0", data_dir, "6-140_MC", [[135, 145]], [100, 200]))
    for run in []: #[9728]:
        analyses.append(get_analysis([run], "2017-02-1 10-140-"+str(run), [27, 30], "MAUS-v3.0.0", data_dir, "10-140_MC", [[135, 145]], [100, 200]))
    for run in [9760]:
        analyses.append(get_analysis([run], "2017-02-2 3-200-"+str(run), [26.5, 29], "MAUS-v3.0.0", data_dir, "3-140_MC", [[195, 205]], [100, 300]))
    for run in [9761]:
        analyses.append(get_analysis([run], "2017-02-2 6-200-"+str(run), [26.5, 28.5], "MAUS-v3.0.0", data_dir, "3-140_MC", [[195, 205]], [100, 300]))
    for run in [9763]:
        analyses.append(get_analysis([run], "2017-02-2 10-200-"+str(run), [26.5, 28.2], "MAUS-v3.0.0", data_dir, "3-140_MC", [[195, 205]], [100, 300]))
    ...
    number_of_spills = 1999 #None #100 # if set to an integer, limits the number of spills loaded for each sub-analysis
