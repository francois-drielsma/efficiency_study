...
def fractional_axis_range(data, fraction):
    ...
    try:
        ...
        xmax = data[-1]
    print data_len, "**", min_index, max_index, "**", xmin, xmax, "**", data[0], data[-1]
    return xmin, xmax
...
text_boxes = []
def get_text_box(config, config_anal, data_list, fit = None):
    ...
    text_box.AddText("Number: "+str(len(data_list)))
