...
class kNNDensityEstimator(object):
    ...
    def __init__(self, data, rotate=True, threads=1, norm=1.):

	# Initialize the number of neighbors following the default prescriptions
	self.data = data
	self.cov = np.cov(data.transpose())
	self.rotate = rotate
	self.n = len(data)
	self.dim = data.shape[1]
	self.k = int(pow(self.n, 4./(4.+self.dim)))
	self.norm = norm
	self.scale = 1.
	...
	profile.GetYaxis().SetTitleOffset(1)
	    ...
	    for i in range(npoints):
	    	...
	    	level = np.quantile(self.levels, 1.-alpha)
		...
		self.__init__(sample, self.rotate, self.threads, self.norm)
		self.set_levels()

		# Evaluate the array of levels of the new sample	        
	    	for i in range(npoints):
	            alpha = (float(i+1)/(npoints+1))
	    	    levels[i][j] = np.quantile(self.levels, 1.-alpha)
	    ...
	    # Reset the data
	    self.__init__(baseline, self.rotate, self.threads, self.norm)
